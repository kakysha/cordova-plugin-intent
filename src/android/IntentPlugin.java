...
public class IntentPlugin extends CordovaPlugin {
	...
	public boolean readFileFromContentUrl(final JSONArray data, final CallbackContext context) {
		if (data.length() != 1) {
			context.sendPluginResult(new PluginResult(PluginResult.Status.INVALID_ACTION));
			return false;
		}
		...
		InputStream is = null;
		try {
			is = cr.openInputStream(Uri.parse(data.getString(0)));
			...
			context.sendPluginResult(new PluginResult(PluginResult.Status.OK, buffer.toByteArray()));
			return true;
		} finally {
			if (is != null) {
				try {
	...
	public boolean getRealPathFromContentUrl(final JSONArray data, final CallbackContext context) {
		...
		Cursor cursor = null;
		try {
			cursor = cR.query(Uri.parse(data.getString(0)), new String[] {
